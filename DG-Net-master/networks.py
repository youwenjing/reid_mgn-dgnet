...
class MsImageDis(nn.Module):
    ...
    def __init__(self, input_dim, params, fp16):
        ...
        self.num_scales = params['num_scales'] # 图片缩放的次数
    ...
    def forward(self, x):
        if not self.gan_type == 'wgan':
            outputs = []
            for model in self.cnns:  # x[4, 3, 256, 128]，注意，输出的结果有3个，因为三个尺寸进行预测
...
class AdaINGen(nn.Module):
    ...
    def __init__(self, input_dim, params, fp16):
        ...
        # MLP to generate AdaIN parameters 产生AdaIN
    ...
    def encode(self, images):
        ...
        return content
    # 调用该函数，进行解码 content为st code ID为ap code
    def decode(self, content, ID):
        ...
        ID3 = ID[:,4096:6144]
        ID4 = ID[:,6144:]
        # 把输入的ap code分成4个部分进行全连接，然后得到四个256维的
