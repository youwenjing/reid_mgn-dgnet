...
class MGN(nn.Module):
    def __init__(self, args):
        ...
        self.fc_id_256_2_3 = nn.Linear(args.feats, num_classes, bias=False)  #！！
    ...
    def forward(self, x):
        ...
        print(list(zg_p1.size()))
        zg_p2 = self.maxpool_zg_p2(p2)
        print(list(zg_p2.size()))
        zg_p3 = self.maxpool_zg_p3(p3)
        print(list(zg_p3.size()))
        ...
        z2_p3 = zp3[:, :, 2:3, :]
        z3_p3 = zp3[:, :, 3:4, :]
        print(list(z3_p3.size()))

        
        fg_p1 = self.reduction_0(zg_p1).squeeze(dim=3).squeeze(dim=2)
        print(list(fg_p1.size()))
        fg_p2 = self.reduction_1(zg_p2).squeeze(dim=3).squeeze(dim=2)
        print(list(fg_p2.size()))
        fg_p3 = self.reduction_2(zg_p3).squeeze(dim=3).squeeze(dim=2)
        print(list(fg_p3.size()))
        f0_p2 = self.reduction_3(z0_p2).squeeze(dim=3).squeeze(dim=2)
        f1_p2 = self.reduction_4(z1_p2).squeeze(dim=3).squeeze(dim=2)
        f2_p2 = self.reduction_5(z2_p2).squeeze(dim=3).squeeze(dim=2)
        print(list(f0_p2.size()))
        f0_p3 = self.reduction_6(z0_p3).squeeze(dim=3).squeeze(dim=2)
        f1_p3 = self.reduction_7(z1_p3).squeeze(dim=3).squeeze(dim=2)
        f2_p3 = self.reduction_8(z2_p3).squeeze(dim=3).squeeze(dim=2)
        f3_p3 = self.reduction_9(z3_p3).squeeze(dim=3).squeeze(dim=2)
        print(list(f0_p3.size()))
        ...
        print(list(l_p1.size()))
        l_p2 = self.fc_id_2048_1(fg_p2)
        l_p3 = self.fc_id_2048_2(fg_p3)
        print(list(l_p3.size()))
        
        l0_p2 = self.fc_id_256_1_0(f0_p2)
        l1_p2 = self.fc_id_256_1_1(f1_p2)
        l2_p2 = self.fc_id_256_1_2(f2_p2)
        print(list(l2_p2.size()))
        l0_p3 = self.fc_id_256_2_0(f0_p3)
        l1_p3 = self.fc_id_256_2_1(f1_p3)
        l2_p3 = self.fc_id_256_2_2(f2_p3)
        l3_p3 = self.fc_id_256_2_3(f3_p3)
        print(list(l3_p3.size()))

        predict = torch.cat([fg_p1, fg_p2, fg_p3, f0_p2, f1_p2, f2_p2, f0_p3, f1_p3, f2_p3, f3_p3], dim=1)

        return predict, fg_p1, fg_p2, fg_p3, l_p1, l_p2, l_p3, l0_p2, l1_p2, l2_p2, l0_p3, l1_p3, l2_p3, l3_p3
