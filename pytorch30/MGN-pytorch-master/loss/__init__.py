...
from loss.triplet import TripletLoss, MSMLLoss, CenterLoss
...
class Loss(nn.modules.loss._Loss):
    def __init__(self, args, ckpt):
        ...
        for loss in args.loss.split('+'):
            ...
            elif loss_type == 'MSMLLoss':
                loss_function = MSMLLoss(args.margin)
            self.loss.append({
                'type': loss_type,
                'weight': float(weight),
    ...
    def forward(self, outputs, labels):
        ...
        for i, l in enumerate(self.loss):
            ...
            elif self.args.model == 'MGN' and l['type'] == 'CenterLoss':
                loss = [l['function'](output, labels) for output in outputs[1:4]]
                loss = sum(loss) / len(loss)
                effective_loss = l['weight'] * loss
                losses.append(effective_loss)
                self.log[-1, i] += effective_loss.item()
            elif self.args.model == 'MGN' and l['type'] == 'MSMLLoss':
                loss = [l['function'](output, labels) for output in outputs[1:4]]
                loss = sum(loss) / len(loss)
                effective_loss = l['weight'] * loss
                losses.append(effective_loss)
