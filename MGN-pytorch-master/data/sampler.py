import random
import collections
from torch.utils.data import sampler

class RandomSampler(sampler.Sampler):
    def __init__(self, data_source, batch_id, batch_image):
        super(RandomSampler, self).__init__(data_source)

        self.data_source = data_source
        self.batch_image = batch_image
        self.batch_id = batch_id

        self._id2index = collections.defaultdict(list) #创建字典
        for idx, path in enumerate(data_source.imgs):
            _id = data_source.id(path)
            self._id2index[_id].append(idx)   #创建索引字典

    def __iter__(self):
        unique_ids = self.data_source.unique_ids
        random.shuffle(unique_ids) #将列表中元素打乱

        imgs = []
        for _id in unique_ids:
            imgs.extend(self._sample(self._id2index[_id], self.batch_image)) #扩展图像列表
        return iter(imgs)

    def __len__(self):
        return len(self._id2index) * self.batch_image

    @staticmethod
    # 从指定序列population中随机获取指定长度k的片断
    def _sample(population, k):
        if len(population) < k:
            population = population * k
        return random.sample(population, k)
