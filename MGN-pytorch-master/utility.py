...
class checkpoint():
    def __init__(self, args):
        ...
        if args.load == '':
            ...
            if not os.path.exists(self.dir):
                ...
                self.log = torch.load(self.dir + '/map_log.pt') #加载对象
                print('Continue from epoch {}...'.format(len(self.log)*args.test_every))

        if args.reset:
            os.system('rm -rf ' + self.dir) #删除原有文件夹
            args.load = ''
        ...
        self.log_file = open(self.dir + '/log.txt', open_type) #写入日志或追加
    ...
    def plot_map_rank(self, epoch):  #绘制测试结果曲线图
...
def make_optimizer(args, model):   #设置加速器
...
def make_scheduler(args, optimizer):  #设置学习率衰减
