...
from functions import cmc, mean_ap
from re_ranking import re_ranking

class Trainer():
    def __init__(self, args, model, loss, loader, ckpt):
        self.args = args                           #输入参数加载
        ...
        self.loss = loss
        self.lr = 0.
        self.optimizer = utility.make_optimizer(args, self.model)        #配置加速器
        self.scheduler = utility.make_scheduler(args, self.optimizer)    #配置学习率
    ...
    def train(self):
        ...
        if lr != self.lr:
            self.ckpt.write_log('[INFO] Epoch: {}\tLearning rate: {:.2e}'.format(epochywj, lr))
            self.lr = lr
        self.loss.start_log()
        self.model.train()
        ...
        for batch, (inputs, labels) in enumerate(self.train_loader):
            ...
            outputs = self.model(inputs)        #训练模型 显示损失
            loss = self.loss(outputs, labels) 
            loss.backward()
            self.optimizer.step()

            self.ckpt.write_log('\r[INFO] [{}/{}]\t{}/{}\t{}'.format(
                epochywj, self.args.epochs,
    ...
    def test(self):
        ...
        if not self.args.test_only:
            self.ckpt.save(self, epochywj, is_best=((best[1][0] + 1)*self.args.test_every == epochywj))
    ...
    def terminate(self):
        if self.args.test_only:
            ...
            epochywj = self.scheduler.last_epoch + 1
            return epochywj >= self.args.epochs
